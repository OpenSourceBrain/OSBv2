...
with workspaces_cli.ApiClient(configuration) as api_client:
    ...
    def add_osbv1_project(osbv1_proj, index):
        ...
        if "GitHub repository" not in osbv1_proj:
            ...
            all_errors.append(
                "  %i, %s doesn't have a Github repo%s..."
                % (index, osbv1_proj_id, but_ok)
            )
            return
        ...
        if found.osbrepositories:
            ...
            url_info = (
                "    OSBv2 repo: https://%s.opensourcebrain.org/repositories/%i (%s)"
                % (v2_or_v2dev, found.osbrepositories[0].id, osbv1_github)
            )
            try:
                print(
