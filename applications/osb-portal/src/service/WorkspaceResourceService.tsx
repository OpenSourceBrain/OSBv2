...
import {
  Configuration,
  WorkspaceResource as ApiWorkspaceResource,
  ResourceType,
  ResourceStatus as ApiResourceStatus,
...
class WorkspaceResourceService {
  ...
  async getResource(id: number): Promise<WorkspaceResource> {
    ...
    const result: ApiWorkspaceResource =
      await WorkspaceService.workspacesApi.workspaceresourceIdGet(wsrigr);
    return mapResource(result);
  }

  async workspacesControllerWorkspaceResourceOpen(id: number): Promise<void> {
    const wsrogr: WorkspacesControllersWorkspaceResourceControllerOpenRequest =
      { id };
    await WorkspaceService.workspacesApi
      .workspacesControllersWorkspaceResourceControllerOpen(wsrogr)
      .then(() => {
        //
      });
  }
  ...
  async deleteResource(resource: WorkspaceResource) {
    return WorkspaceService.workspacesApi.workspaceresourceIdDelete({
      id: resource.id,
    });
  }
  ...
  getResourcePath(resource: WorkspaceResource) {
    return (
      ...
      resource.origin.path.slice(resource.origin.path.lastIndexOf("/") + 1)
    );
  }
}
...
function mapResourceStatus(resource: ApiWorkspaceResource): ResourceStatus {
  switch (resource.status) {
    case ApiResourceStatus.A:
      return ResourceStatus.available;
    case ApiResourceStatus.E:
...
export function mapPostUrlResource(
  resource: WorkspaceResource
): ApiWorkspaceResource {
  return {
    ...resource,
...
export function mapPostAddedResource(
  resource: WorkspaceResource
): ApiWorkspaceResource {
  return {
    ...resource,
