...
const RepositoryPageDetails = () => {
    ...
    return <RepoDetailsPageBox className='verticalFit'>
        <Grid container sx={{ height: '100%', overflow: 'hidden' }} m={0}>
            <Grid item xs={12} md={6} sx={{ display: 'flex', flexDirection: 'column' }}>
                <Box width={1}>
                    <Box display='flex' justifyContent='space-between' alignItems='center' width={1} borderBottom={`1px solid ${lineColor}`}>
                        ...
                        <Button variant='text' endIcon={<ModeEditIcon />} sx={{ textTransform: 'none', padding: 0 }}>Edit</Button>
                    </Box>
                    <Stack mt={3} mb={3} spacing={1}>
                        <Typography variant='body2' sx={{ color: greyishTextColor }}>Context: {`${first}`}</Typography>
                        <Typography variant='body2' sx={{ color: greyishTextColor }}>Created: {`${second}`}</Typography>
                        <Stack direction='row' spacing={1}>
                            <RepoDetailsChip label='main' icon={<LanOutlinedIcon sx={{ fill: paragraph }} />} />
                            <RepoDetailsChip label='Experimental Data' icon={<CircleIcon className='greenStatusDot' />} />
                            <RepoDetailsChip label='Modeling' icon={<CircleIcon className='purpleStatusDot' />} />
                        </Stack>
                    </Stack>
                </Box>
                <Box width={1}>
                    <Box display='flex' justifyContent='space-between' alignItems='center' width={1} borderBottom={`1px solid ${lineColor}`}>
                        ...
                        <Button variant='text' endIcon={<OpenInNewIcon />} sx={{ textTransform: 'none', padding: 0 }}>View on GitHub</Button>
                    </Box>
                    <AboutOSBBox width={1} mt={2} mb={4}>
                        ...
                        <Typography>This repository will contain some example datasets which we will convert to NWB to test this functionality.</Typography>
                        <Typography>The target format will be NWB v2.0 and we intend to make use of PyNWB for reading/writing the NWB files.</Typography>
                    </AboutOSBBox>
                </Box>
            </Grid>
            <Grid item xs={12} md={6} sx={{ display: 'flex', flexDirection: 'column' }}>
                <Box width={1}>
                    ...
                    <Box width={1} display='flex'>
                        ...
                        />
                        <RepoDetailsButton startIcon={<FilterListIcon />} variant='contained'>All Types</RepoDetailsButton>
                    </Box>
                </Box>
                <Box width={1} mt={4}>
                    <Paper elevation={0} sx={{ width: '100%', mb: 2, background: 'none' }}>
                                ...
                                data.map((row, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <ListItemText primary={row.date} sx={{ textAlign: 'right' }} />
                                        </RepoDetailsListItemButton>
                                    )
                                })
                            }
