...
export const EditRepoDialog = ({ dialogOpen, setDialogOpen, onSubmit, repository = RepositoryService.EMPTY_REPOSITORY, title = "Add repository", user }:
  { dialogOpen: boolean, onSubmit: () => any, setDialogOpen: (open: boolean) => any, repository?: OSBRepository, title: string, user: UserInfo }) => {
  const classes = useStyles();

  const [formValues, setFormValues] = useState({ ...repository, userId: user.id })
  const [error, setError] = useState({ uri: false, defaultContext: false, contentTypesList: false, name: false })
  ...
  const addRepository = () => {
    const errors = {
      ...
      contentTypesList: Boolean(formValues.contentTypesList) && formValues.contentTypesList.length === 0,
  ...
  return (
    ...
    >
      ...
      <DialogContent>
              ...
              >
                {
                  Object.values(RepositoryType).
                    ...
                    map((repositoryType) => <MenuItem key={repositoryType} value={repositoryType}>{repositoryType}</MenuItem>)
                }
              </Select>
            </FormControl>
        ...
        <Box className="form-group">
          ...
          >
            ...
            >
              <MenuItem value={RepositoryContentType.Experimental}>NWB Experimental Data</MenuItem>
              <MenuItem value={RepositoryContentType.Modeling}>Modeling</MenuItem>
            </Select>
          </FormControl>
        </Box>
      ...
      <DialogActions>
        ...
        <Button variant="contained" disableElevation={true} onClick={addRepository} color="primary">
          Add
        </Button>
        {loading &&
          <CircularProgress
