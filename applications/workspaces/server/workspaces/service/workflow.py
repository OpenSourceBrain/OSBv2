...
def delete_resource(workspace_resource, pvc_name, resource_path: str):
    resources = {"requests": {"memory": "25Mi", "cpu": "10m"}, "limits": {"memory": "512Mi", "cpu": "100m"}}
...
def download_workspace_resource(workspace_resource, pvc_name, path, folder):
    ...
    download_task = tasks.CustomTask(
        name="osb-download-file",
    ...
    op = operations.PipelineOperation(
        basename=f"osb-download-file-job",
        tasks=(download_task,),
        shared_directory=shared_directory,
        folder=folder,
        pod_context=operations.PodExecutionContext("workspace", workspace_resource.workspace_id),
        on_exit_notify={"queue": "osb-download-file-queue", "payload": workspace_resource.id},
    )
    workflow = op.execute()
