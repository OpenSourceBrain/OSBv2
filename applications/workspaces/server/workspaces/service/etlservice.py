...
def download_workspace_resource(workspace_resource):
    try:
        ...
        workflow.download_workspace_resource(
            workspace_resource=workspace_resource, pvc_name=pvc_name, path=path, folder=workspace_resource.folder
        )
    except Exception as e:
        logger.error("An error occurred while adding the default resource to the workspace", exc_info=True)
        return workspace_resource
    return workspace_resource
...
def delete_workspace_resource(workspace_resource):
    try:
        ...
        logger.error("An error occurred while deleting resource from the workspace", exc_info=True)
        return None
    return True
